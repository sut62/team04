        ...
        <div class="LoginPage" >
         <v-card>
          ...
          <v-row justify="center" class="pb-12 mb-10" >
            <v-col class="ml-10" cols="12" sm="2" md="-2">
              <v-btn
              @click="Login"
              class="cyan lighten-1 white--text"
            >
            Login<v-icon size="30px" right>mdi-key-variant</v-icon>
            </v-btn>
            </v-col>
            <v-col class="ml-10" cols="12" sm="2" md="-2">
              <v-btn
              ...
              class="cyan lighten-1 white--text"
            >
            Register Customer<v-icon size="30px" right>mdi-account-plus</v-icon>
            </v-btn>
            </v-col>
          </v-row>
  ...
  alert("Plese input all data");
     ...
     http
        ...
        .then(response => {
          ...
          if (this.login.Username == response.data.em_name && this.login.Password == response.data.password) {
            this.employee = response.data;
            this.login.Username = response.data.em_name;
            this.login.Password = response.data.password;
            // alert(this.login.Username)
            alert("Login complete");
            this.$router.push({name: 'Dashbord' , params: {em: this.employee.em_id} }); 
          } 
            else {
    ...
    .then(response=>{
       ...
       if (this.login.Username == response.data.Email && this.login.Password == response.data.password) {
